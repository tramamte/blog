...
[Semicolons](https://tramamte.github.io/2018/05/29/semicolons/) 글과 관련된 fun fun한 글입니다.
...
어떤 코드가 더 아름다운가에 대한 케케묵은 논쟁을 겨우 세미콜론만으로 꺼내자는 것은 아니지만, 개인적으로는 *"모든 가능성은 열려 있어. 너의 꿈을 펼쳐봐"* 를 주장하는 Perl보다는 *"이 길이 꽃길이야. 이 길로만 가면 아름다운 세상이 펼쳐질거야"* 를 주장하는 Python의 철학을 좀 더 지지하는 편이다. - *Python이 이상한 언어처럼 보이고, 나는 꼰대로 보이는 것은 왜 때문이지?*
...
'어떤 코드가 아름다운가?'는 각자의 답이 다른겠지만 '어떤 코드가 보기 좋은가(편한가)?'에 대한 답은 대부분의 개발자가 비슷하지 않을까?